...
server.registerTool(
  'browse_use',
  {
    ...
    description:
      ...
      'Provide a clear task and optionally a start URL. Returns the agent\'s findings plus a run summary. ' +
      'Blocks until the task finishes (up to several minutes for multi-step tasks).',
    inputSchema: {
      ...
      app: z.string().optional().describe('Optional app name (e.g. "acme-crm") to reuse a saved login session, learned skills, and credentials for that app.'),
    },
  },
  async ({ task, url, app }) => {
    let res;
    ...
    // Fast path: engine finished within its initial 2s window.
